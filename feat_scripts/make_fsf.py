...
def create_fsf(basedir,repl_dict,outdir, arglist):
    ...
    for sub in glob.glob('sub-*'):
        ...
        print(sub)
        for scan in glob.glob(os.path.join(sub,'func','%s_task-%s_bold_brain_mcf.nii.gz')%(sub,arglist['TASK'])):
            print(scan)
            ...
            for item in arglist['EV']:
                print(item)
                ctr=ctr+1
                ev=os.path.join(basedir,sub,'func','onsets','%s_%s_%soutput.txt'%(sub,arglist['TASK'],item))
